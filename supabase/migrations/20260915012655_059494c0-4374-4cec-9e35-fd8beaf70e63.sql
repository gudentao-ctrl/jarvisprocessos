...
BEGIN
  ...
  WHERE lower(btrim(name)) = lower(btrim(_lead.company_name))
  ORDER BY created_at
  LIMIT 1;
  ...
  ELSE
    UPDATE public.companies SET is_active = true WHERE id = _company_id;
  END IF;
...
END;
$$;
REVOKE ALL ON FUNCTION public.convert_crm_lead_to_company(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.convert_crm_lead_to_company(uuid) TO authenticated, service_role;