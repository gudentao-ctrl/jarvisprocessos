...
export const listOpportunities = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      ...
      .select("*, processes(name), companies(name), pain_points(description), indicators(name)")
...
export const getOpportunity = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row, error } = await context.supabase
      .from("improvement_opportunities")
      .select("*, processes(id,name), companies(id,name), pain_points(id,description), indicators(id,name), root_cause_analyses(id,problem,method), action_plans(id,title,status)")
...
export const listRcas = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      ...
      .select("*, companies(name), processes(name), pain_points(description)")
      .order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
    return data ?? [];
  });
...
export const getRca = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row, error } = await context.supabase
      .from("root_cause_analyses")
      .select("*, companies(id,name), processes(id,name), pain_points(id,description)")
      .eq("id", data.id).single();
    if (error) throw new Error(error.message);
