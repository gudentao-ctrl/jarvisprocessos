  ...
  .handler(async ({ data }) => {
    ...
    const [indicators, plans, processes] = await Promise.all([
      ...
      sb
        .from("action_plans")
        .select(
          "id, title, problem, cause, category, responsible, priority, status, due_date, new_due_date, expected_result, observations, origin, created_at, updated_at, process_id",
        )
        .eq("company_id", companyId)
...
export const getPublicPlanDetails = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: plan, error: pe } = await sb
      .from("action_plans")
      .select(
        "id, title, problem, cause, category, responsible, priority, status, due_date, new_due_date, expected_result, observations, evidences, origin, created_at, updated_at",
      )
      .eq("id", data.plan_id)
