...
function ImpactPage() {
  ...
  const causes = new Map(data.causes.map((row: any) => [row.id, row]));
  const pains = new Map(data.pains.map((row: any) => [row.id, row]));
  const interviews = new Map(data.interviews.map((row: any) => [row.id, row]));
